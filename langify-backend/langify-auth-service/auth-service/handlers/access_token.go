...
// AccessTokenHandler generates a new access token using the refresh token
func AccessTokenHandler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	// Parse request body
	var reqData struct {
	...
	if err != nil {
		...
		return events.APIGatewayProxyResponse{StatusCode: 500, Body: `{"error": "Failed to generate new access token"}`}, nil
	}

	// Update session in Redis with the new access token
	err = database.StoreSession(reqData.Email, newAccessToken, storedRefreshToken)
	if err != nil {
		log.Printf("Failed to update session in Redis: %v", err)
		return events.APIGatewayProxyResponse{StatusCode: 500, Body: `{"error": "Failed to store new access token"}`}, nil
	}
